...
Engine.register('xiangqi', function (engine, cfg) {
  ...
  let gameOn = false;
  let sel = -1, selMoves = [], lastMv = null;
  ...
  function drawPlay(ctx) {
    ...
    if (sel >= 0) {
      ...
      if (SET.m) for (let i = 0; i < selMoves.length; i++) {
        ...
        else { ctx.fillStyle = 'rgba(120,220,130,0.8)'; ctx.beginPath(); ctx.arc(X, Y, cell * 0.14, 0, 7); ctx.fill(); }
      }
    }
  ...
  function drawHud(ctx) {
    ...
    ctx.fillText('двойной тап по фигуре — подсказка', W / 2, H - 6);
  }
  ...
  function tapBoard(x, y) {
    ...
    if (cellI >= 0 && bd[cellI] && isDouble) {
      ...
      // заодно подсветим ходы этой фигуры
      if (side(bd[cellI]) === 'r' && turn === 'r' && gameOn) {
        sel = cellI;
        selMoves = legalMoves(bd, false).filter(function (m) { return ((m / 90) | 0) === cellI; });
      }
      snd(600, 0.06, 'sine', 0.08);
      lastTap.t = 0;
      return;
    }
    if (!gameOn || turn !== 'r' || anim || thinking) return;
  ...
  return {
    ...
    _state: function () {
      return { ui: ui, turn: turn, gameOn: gameOn, board: serialize(), mc: mc, idle: idle,
        sel: sel, nMoves: selMoves.length, capR: capR.join(''), capB: capB.join(''),
